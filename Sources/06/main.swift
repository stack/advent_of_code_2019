...
let parsedData = data.map { (value: String) -> (String, String) in
    ...
    let orbitee = String(parts[0])
    let orbiter = String(parts[1])

    return (orbitee, orbiter)
}

// For each object, add it empty to storage
for (orbitee, orbiter) in parsedData {
    objects[orbitee] = Object(name: orbitee)
    objects[orbiter] = Object(name: orbiter)
}

// Link each object in storage to its orbit
for (orbitee, orbiter) in parsedData {
    objects[orbiter]?.orbits = objects[orbitee]
}
